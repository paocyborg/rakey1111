...
RfuBot=[lineMID]
Family=["u7a5592b486c643f1f60a444854037e28",lineMID]
admin=['u7a5592b486c643f1f60a444854037e28',lineMID]
...
settings = {
    ...
    "pname": {},
    "pro_name": {},
    "message":"บัญชีนี้ได้การป้องกันโดย ചัঐのุঞ☬ʄທയஆടஷະ☬のລັப ระบบได้ทำการบล็อคคุณอัตโนมัติ",
...
#==============================================================================#
                    ...
                    me = line.getContact(lineMID)
                    line.sendMessage(msg.to,"[นี้ไงตัสที่ใส่ยุ]\n" + me.statusMessage)
                elif text.lower() == 'รูป':
                    me = line.getContact(lineMID)
...
#==============================================================================#
                    ...
                    for target in targets:
                        try:
                            ...
                            break
                elif msg.text.lower().startswith("ลบmimic "):
                    targets = []
                    key = eval(msg.contentMetadata["MENTION"])
                    key["MENTIONEES"][0]["M"]
                    for x in key["MENTIONEES"]:
...
#==============================================================================#
                    ...
                    if msg.toType == 2:
                        group = line.getGroup(to)
                        ret_ = "╔══[ รายชื่อสมาชิกห้องนี้ ]"
                        no = 0 + 1
                        for mem in group.members:
                            ret_ += "\n╠ {}. {}".format(str(no), str(mem.displayName))
                            no += 1
                        ret_ += "\n╚══[ หมดละ {} ]".format(str(len(group.members)))
                        line.sendMessage(to, str(ret_))
                elif text.lower() == 'กลุ่มเรา':
                        groups = line.groups
                        ret_ = "╔══[ รายชื่อกลุ่มที่เรามี ]"
                        no = 0 + 1
                        for gid in groups:
                            group = line.getGroup(gid)
                            ret_ += "\n╠ {}. {} | {}".format(str(no), str(group.name), str(len(group.members)))
                            no += 1
                        ret_ += "\n╚══[ หมดละ {} Groups ]".format(str(len(groups)))
                        line.sendMessage(to, str(ret_))				
...
#==============================================================================#   
                ...
                elif msg.text in ["ไอดีเพื่อนทั้งหมด"]: 
...
#=============COMMAND PROTECT=========================#
                    ...
                    if RfuProtect["Protectguest"] == True:
                        if settings["lang"] == "JP":
                            line.sendMessage(msg.to,"เปิดป้องกันสมาชิกเข้า   ")
                        else:
                            line.sendMessage(msg.to,"เปิดป้องกันสมาชิกเข้า   ")
                    else:
